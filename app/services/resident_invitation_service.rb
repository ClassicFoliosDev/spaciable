...
module ResidentInvitationService
  ...
  def call(plot_residency, from_user, invited_by)
    return unless plot_residency

    @plot = plot_residency.plot

    if plot_residency.invitation_accepted_at.nil?
      new_resident(plot_residency, from_user, invited_by)
    else
      existing_resident(plot_residency, invited_by)
    end
  end
  ...
  def new_resident(plot_residency, from_user, invited_by)
    ...
    send_reminders(plot_residency, invited_by)
  end

  def send_reminders(plot_residency, invited_by)
    ...
    InvitationReminderJob.set(wait: 1.week)
                         .perform_later(plot_residency, subject, token, invited_by)
  ...
  def existing_resident(plot_residency, invited_by)
    NewPlotJob.perform_later(plot_residency, I18n.t(".new_plot_title"), invited_by)
  end
end
