...
When(/^I update the developer$/) do
  ...
  find("#developer_house_search").trigger(:click)
...
Then(/^I should see the delete complete successfully$/) do
  ...
  within ".empty" do
    expect(page).to have_content %r{#{t("components.empty_list.add", action: "Add", type_name: Developer.model_name.human)}}i
  end
end
...
Then(/^I should (not )*see CAS visable and enabled at the development$/) do |not_visible|
  ...
  visit "/developers/#{development.developer.id}/developments/#{development.id}/edit"
