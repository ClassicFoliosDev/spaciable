...
Then(/^I should see the branding for my page$/) do
  ...
  style_contents = style['outerHTML']
  ...
  expect(style['outerHTML']).to have_content("branded-content {\n      background-color: #32344E")
  ...
  expect(style['outerHTML']).not_to have_content("branded-btn {\n      color: #A6A7B2")
...
Then(/^I should see the configured branding$/) do
  ...
  within ".preamble" do
    ...
    expect(page.body).to have_content "branded-content {\n      background-color: #32344E"
    # Text color should be text color, set on developer only
    expect(page.body).to have_content "branded-text {\n      color: #646467"
    # Title color should be button (background) color, set on development and developer: should be development color
    expect(page.body).to have_content "branded-titles h2 {\n      color: #776644"
    expect(page.body).to have_content "branded-titles h2:last-child {\n      color: #776644"
  end

  # Label text should be text color, set on development and developer: should be development color
  expect(page.body).to have_content "branded-primary-text {\n      color: #776644"
  # Input field background should be content bg color, set on developer and division: should be division
  expect(page.body).to have_content ".branded-input input:-webkit-autofill {\n      -webkit-box-shadow: 0 0 0 30px #32344E"
  # Input field border should be content border color, set on development only
  expect(page.body).to have_content ".branded-input input[type=\"email\"] {\n      border-color: #446677"
  expect(page.body).to have_content ".branded-input input[type=\"password\"] {\n      border-color: #446677"
  # Link text should be button (background) color as text colour, set on development and developer: should be development color
  expect(page.body).to have_content ".branded-primary-text label.checkbox {\n      color: #776644"
  expect(page.body).to have_content ".branded-primary-text a {\n      color: #776644"
  # Button text should be button (text) color, set on development, division and developer: should be development color
  expect(page.body).to have_content "branded-btn {\n      color: #698492"
